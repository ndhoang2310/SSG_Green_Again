# GREEN AGAIN V6 — DOCUMENT TỔNG HỢP VÀ HƯỚNG DẪN BUILD

> **Phiên bản:** 1.0 — 2026-07-02
> **Mục đích:** Một document duy nhất đóng vai trò Game Design Document, Build Guide, Implementation Reference, UI/UX Spec và NPC/Dialogue/Cutscene/Movement Spec cho Green Again V6.
...
## V6 DESIGN DELTA - PHẢI ĐỌC TRƯỚC KHI IMPLEMENT

V6 kế thừa toàn bộ nền v5, nhưng không dừng ở cao trào dọn cống. Phần sau Chapter 4 phải được phát triển thêm để tận dụng khu dân cư, cây xanh, và một beat nhỏ về động vật/chỗ sống.

**Implementation target hiện tại:** V6 đang được build như một expansion trên root `Workspace["=== GREEN AGAIN V5 ==="]` và hai script active `StoryRuntimeMVP` / `StoryClientMVP`. Không rename root sang `=== GREEN AGAIN V6 ===` trong pass này; mọi path V6 root cũ chỉ là future migration note.

Core change:

```text
Ch1-4 giữ nguyên mạch v5
-> Ch5 dọn cống
-> gọi cộng đồng
-> quay lại ngõ dân cư sau mưa
-> chăm một góc sống nhỏ cho chim/mèo/cún bằng cleanup + placement nhẹ
-> trồng cây non ở các vị trí có ý nghĩa
-> ending Nhà văn hóa với cam kết cụ thể hơn
```

Quest IDs mới của Ch5:
...
- Không thêm hệ thống cứu hộ/phức tạp cho động vật. Beat `CareSmallLives` dùng cleanup, placement, visual toggle, dialogue reaction, hoặc sound cue.
- Khu dân cư phải có vai trò thật ở cuối game: Cô Hạnh, Em Phúc, ngõ sau mưa, đường đi học, gốc cây.
- Trồng cây phải có chủ sở hữu story: Em Phúc/Bé Na/Cô Hạnh nhận chăm cây, không chỉ tăng placement count.
- Nếu scope code nhỏ, gộp các beat mới vào handler cleanup/placement hiện có, nhưng vẫn giữ objective/dialogue riêng.

---

## 1. TỔNG QUAN GAME

**Nguồn chính:** `GDD_GreenAgain_v6.md`, `GDD_GreenAgain_v6_STORY_BIBLE.md`
...
- Kết thúc bằng cộng đồng cùng nhận phần việc nhỏ, bao gồm giữ ngõ dân cư, chăm cây non và chừa lại một góc sống sạch cho những sinh vật nhỏ quanh làng.
...
5. **Hy vọng:** Cộng đồng bắt đầu cùng thay đổi, và player thấy thay đổi đó ở người dân, cây non, đường đi học, và một góc sống nhỏ sạch hơn.

---

## 2. PILLARS THIẾT KẾ

**Nguồn chính:** `GDD_GreenAgain_v6.md` (Section 2), `GDD_GreenAgain_v6_STORY_BIBLE.md` (Section 3)
...
- Cây xanh và động vật không phải phần trang trí; chúng là dấu hiệu cho thấy nơi này còn đáng sống.
...
**Nguồn chính:** `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`
...
| Đã xóa | Lý do |
|---|---|
| `StarterGui.GreenAgainHUD_V6` | HUD cũ gây duplicate UI |
...
**Không còn:** Old top-left text `Đang nối map Green Again V6...`, old stamina bar.
...
**Nguồn chính:** `GDD_GreenAgain_v6_CURRENT_MAP_DRAFT.md`, `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`

### 4.1 Root

| Path | Mô tả |
|---|---|
| `Workspace["=== GREEN AGAIN V5 ==="]` | Root folder hiện tại chứa map; V6 expansion chạy trên root này |
...
|---|---|---|---|---|---|
| `BacXanh` | `MAP_ROUTE.06_NPCs.BacXanh` | Bác Xanh | Mentor, người giữ ký ức Ven Sông, dẫn player qua story | Q1_02, transitions, Q5_06-08, ending | NPC chính, interactable nhiều nhất |
...
| `AnhTung` | `MAP_ROUTE.06_NPCs.AnhTung` | Anh Tùng | Thanh niên chơi bóng, thói quen "lát dọn" | Q3_01-06, Q5_04, ending | Đứng ở sân bóng |
| `BeNa` | `MAP_ROUTE.06_NPCs.BeNa` | Bé Na | Em nhỏ, đại diện thế hệ sau | Q3_02-04, ending | Gần sân bóng |
| `OngSau` | `MAP_ROUTE.06_NPCs.OngSau` | Ông Sáu | Người sống lâu bờ sông, hoài niệm | Q4_01-04, Q5_04, ending | Gần bờ sông |
| `CoHanh` | NPC phụ mới hoặc reused villager | Cô Hạnh | Tổ dân phố, giữ ngõ dân cư | Q5_05, ending | Có thể dùng runtime/staged NPC nếu chưa có model riêng |
| `EmPhuc` | NPC phụ mới hoặc reused child villager | Em Phúc | Học sinh đi qua ngõ mỗi ngày | Q5_05-07, ending | Có thể đứng cạnh Bé Na ở ending |

### 4.3 Location Objects

| Object | Path gần đúng | Tên hiển thị | Vai trò story | Quest liên quan | Ghi chú |
|---|---|---|---|---|---|
| `NhaVanHoa_ThonNoiChuan` | Trong `MAP_ROUTE` hoặc root V6 | Nhà văn hóa thôn Ven Sông | Hub khởi đầu, nơi gặp Bác Xanh, ending | Q1_01, Q1_02, Q5_08, ending | Hub trung tâm |
...
| `ResidentialLane` | Runtime anchor trên `PROPS_AND_INTERACTABLES_REVIEW.Loose_Props.duong` | Ngõ dân cư sau mưa | Tuyến Ch5: Cô Hạnh/Em Phúc, đường đi học, rác đọng sau mưa | Q5_05 | Tọa độ `(250.00, 48.90, -3370.00)` |
| `SmallLivesCorner` | Runtime anchor trên cùng đoạn `duong` | Góc sống nhỏ | Dọn rác nguy hiểm, đặt bát nước/biển nhỏ, optional prop/sound chim/mèo/cún | Q5_06 | Tọa độ `(246.00, 48.90, -3385.00)`; không cần rescue system |
...
| Tên kỹ thuật cũ | Tên hiển thị V6 | Lưu ý |
|---|---|---|
...
**Nguồn chính:** `GDD_GreenAgain_v6_QUEST_FLOW.md`, `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`
...
### 5.5 Chapter 5 — Cống Cuối Xóm Và Những Chỗ Sống Nhỏ

**Story question:** Làm sao để chuyện không lặp lại, và ai được hưởng khi Ven Sông xanh hơn?
**Main NPC:** Bác Xanh, cộng đồng, Cô Hạnh, Em Phúc
**Notebook:** "Dọn cống là xử lý hậu quả. Để rác không quay lại, cả làng phải đổi thói quen nhỏ của mình: giữ ngõ sạch, chừa chỗ sống cho những sinh vật nhỏ, và chăm cây non mỗi ngày."
...
| **Runtime object** | Không |
| **Quest tiếp theo** | `Q5_05_ResidentialAfterRain` |
...
| Mục | Chi tiết |
|---|---|
| **Quest ID** | `Q5_05_ResidentialAfterRain` |
| **Chapter** | 5 |
| **Title** | Ngõ Sau Cơn Mưa |
| **Objective** | Gặp Cô Hạnh và Em Phúc ở khu dân cư |
| **Target** | `ResidentialLane` runtime anchor hoặc đoạn thật trong `ResidentialSt` |
| **Interaction type** | Talk + inspect |
| **Completion condition** | Dialogue complete + inspect/cleanup 2 điểm thấp |
| **Dialogue** | `CoHanh_EmPhuc_Q5_ResidentialAfterRain` |
| **Marker** | Marker tại Cô Hạnh / ngõ dân cư |
| **Runtime object** | 2 inspect/cleanup spots: mương/ngõ đọng rác, đường đi học sau mưa |
| **Quest tiếp theo** | `Q5_06_CareSmallLives` |
| **Notes** | Tận dụng khu dân cư để cho thấy hậu quả quay lại đời sống hằng ngày |

#### Q5_06_CareSmallLives

| Mục | Chi tiết |
|---|---|
| **Quest ID** | `Q5_06_CareSmallLives` |
| **Chapter** | 5 |
| **Title** | Chỗ Sống Nhỏ |
| **Objective** | Làm sạch góc cây và đặt bát nước/biển nhỏ |
| **Target** | `SmallLivesCorner` runtime anchor gần gốc cây/bụi cỏ |
| **Interaction type** | Cleanup + placement/care |
| **Completion condition** | Care action count reached |
| **Dialogue** | `SmallLives_Q5_Care` |
| **Marker** | Marker tại góc cây |
| **Runtime object** | Dây nilon/mảnh nhựa, bát nước hoặc biển nhỏ; optional prop/sound cue chim/mèo/cún sau khi xong |
| **Quest tiếp theo** | `Q5_07_NeighborhoodPlanting` |
| **Notes** | Không cần rescue/AI phức tạp. Beat này nói rằng chừa lại một chỗ sạch cũng là giúp sự sống quay về |

#### Q5_07_NeighborhoodPlanting

| Mục | Chi tiết |
|---|---|
| **Quest ID** | `Q5_07_NeighborhoodPlanting` |
| **Chapter** | 5 |
| **Title** | Mỗi Nhà Một Bóng Cây |
| **Objective** | Trồng cây non và đặt phòng ngừa ở khu làng |
| **Target** | Placement zones: ngõ dân cư, Nhà văn hóa, bờ sông/sân bóng |
| **Interaction type** | Placement |
| **Completion condition** | Placement count reached |
| **Dialogue** | `NeighborhoodPlanting_Q5` intro/after lines |
| **Marker** | Marker lần lượt tại các placement spots |
| **Runtime object** | Runtime placement markers: cây non, thùng rác/biển nhắc nếu cần |
| **Quest tiếp theo** | `Q5_08_ReturnToCommunityHouse` |
| **Notes** | Reuse runtime tree visual hiện có; cây phải có ý nghĩa story, không chỉ là decoration |

#### Q5_08_ReturnToCommunityHouse

| Mục | Chi tiết |
|---|---|
| **Quest ID** | `Q5_08_ReturnToCommunityHouse` |
| **Chapter** | 5 |
...
| **Notes** | CS5E trigger. Ending fires once only |
...
**Nguồn chính:** `GDD_GreenAgain_NPC_DIALOGUE_v6.md`
...
- **Dialogue ID:** `CoHanh_EmPhuc_Q5_ResidentialAfterRain`
- **Quest:** `Q5_05_ResidentialAfterRain`
- **Purpose:** Tận dụng khu dân cư và cho thấy hậu quả quay lại đường đi học/ngõ nhà.

```
Cô Hạnh: Con vừa ở cống về hả? Vậy con nhìn thử ngõ này sau mưa đi.
Player: Cháu tưởng cống nghẹt thì chỉ ảnh hưởng ở cuối xóm.
Cô Hạnh: Không đâu. Nước dềnh lên là rác quay ngược về mấy đoạn thấp.
Em Phúc: Con đi học ngang đây mỗi ngày. Có bữa mưa xong, vỏ bánh với túi nilon dồn ngay chỗ con bước qua.
Player: Vậy khu dân cư cũng là một phần của đường rác.
Cô Hạnh: Ừ. Trước giờ cô hay nhắc chung bằng loa. Nhưng nhắc chung thì dễ trôi đi như nước.
Em Phúc: Nếu mỗi nhà giữ đoạn trước cửa mình, tụi con đi học đỡ phải né rác.
```
...
- **Dialogue ID:** `SmallLives_Q5_Care`
- **Quest:** `Q5_06_CareSmallLives`
- **Purpose:** Làm rõ động vật/chỗ sống mà không cần rescue system.

```
Em Phúc: Hồi trước chỗ gốc cây này hay có chim xuống uống nước.
Bé Na: Em cũng từng thấy một con mèo nằm ở đây. Mấy hôm nay nó tránh ra xa.
Player: Chắc vì rác mắc quanh gốc cây?
Em Phúc: Dạ. Có cả dây nilon nữa. Nhìn nhỏ thôi, nhưng tụi nó đâu biết gỡ như mình.
Bác Xanh: Không phải lúc nào mình cũng cần làm chuyện lớn như cứu một con vật.
Bác Xanh: Nhiều khi chỉ cần chừa lại một chỗ sạch để sự sống tự quay về.
```
...
- **Dialogue ID:** `NeighborhoodPlanting_Q5`
- **Quest:** `Q5_07_NeighborhoodPlanting`
- **Purpose:** Biến trồng cây thành lời hứa có người chăm.

```
Cô Hạnh: Dọn xong rồi, mình cần thứ gì đó để mọi người nhớ chăm tiếp.
Player: Trồng cây được không cô?
Cô Hạnh: Được. Một cây ở đầu ngõ, một cây gần Nhà văn hóa, một cây gần bờ sông.
Chị Lan: Trồng cây không thay thế chuyện bỏ rác đúng chỗ. Nhưng nó nhắc mọi người rằng chỗ sạch cần được chăm mỗi ngày.
Em Phúc: Con nhận tưới cây đầu ngõ. Nhưng nếu con quên, cô Hạnh nhắc con nha.
```
...
Cô Hạnh: Tổ dân phố sẽ chia việc nhỏ theo từng đoạn ngõ. Không đợi tới lúc rác dồn lại mới gọi nhau đi dọn.
Em Phúc: Con nhận tưới cây đầu ngõ. Con cũng nhắc mấy bạn đừng đá vỏ bánh xuống mương.
Bé Na: Con với Phúc sẽ coi góc cây nữa. Nếu chim hay mèo quay lại, tụi con giữ chỗ đó sạch.
Bác Xanh: Nghe chưa? Không ai nhận làm hết. Mỗi người nhận một phần nhỏ.
Bác Xanh: Ngày trước Ven Sông xanh vì người ta nhớ phần nhỏ đó.
Bác Xanh: Từ hôm nay, mình nhớ lại.
Player: Ven Sông sẽ xanh lại chứ bác?
...
**Nguồn chính:** `GDD_GreenAgain_NPC_DIALOGUE_v6.md` (Section 11)
...
| **Cô Hạnh** (tổ dân phố) | "Nhắc thì ai cũng gật..." | "Từ mai cô chia từng đoạn ngõ..." |
| **Em Phúc** (học sinh) | "Con đi học ngang đây mỗi ngày..." | "Con sẽ nhắc mấy bạn... rồi tưới cây đầu ngõ nữa." |
...
**Nguồn chính:** `GDD_GreenAgain_v6_STORY_BIBLE.md` (Section 5), `GDD_GreenAgain_NPC_DIALOGUE_v6.md`, `GDD_GreenAgain_v6_CUTSCENE_SCRIPT.md`
...
| **Quest xuất hiện** | Q1_02, chapter transitions, Q5_02-08, ending |
| **Interactable khi** | Q1_02 (intro), sau mỗi chapter clean (transition), Q5_08 (ending) |
...
| **Q5_04 Gather** | Player quay lại tạp hóa gặp Cô Tư ở vị trí ban đầu; cô nhận phần giảm túi và nhắc khách | — |
...
| **Quest xuất hiện** | Q3_01-06, Q5_04, ending |
...
| **Q5_04 Gather** | Player quay lại sân bóng gặp Anh Tùng ở vị trí ban đầu; anh nhận luật dọn sân sau trận | — |
...
| **Quest xuất hiện** | Q4_01-04, Q5_04, ending |
...
| **Q5_04 Gather** | Player quay lại bờ sông gặp Ông Sáu ở vị trí ban đầu; ông nhận phần nhắc người ở bờ sông | — |
| **Ending** | Đứng trong nhóm NPC tại Nhà văn hóa | Teleport về NVH |
...
**Nguồn chính:** `GDD_GreenAgain_v6_CUTSCENE_SCRIPT.md`
...
| **MVP implementation** | `Đã build cinematic pass` — CS0 plays once after teleport, with fade from black and 3 camera beats |
...
| **MVP implementation** | `Đã build cinematic pass` — CS1 uses multi-shot camera focus before dialogue |
...
| **MVP implementation** | `Đã build cinematic pass` — multi-shot focus on sorting station |
...
| **MVP implementation** | `Đã build cinematic pass` — camera sweep around tạp hóa |
...
| **MVP implementation** | `Đã build cinematic pass` — wide field shot and focus beats |
...
| **MVP implementation** | `Đã build cinematic pass` — river pan/focus beats |
...
| **MVP implementation** | `Đã build cinematic pass` — low drain camera beats |
...
| **MVP implementation** | `Đã build cinematic pass` — reflective after-drain camera beats |
| **`Đề xuất bổ sung` Polish** | Nước đổi từ tối sang trong. Ánh sáng nhẹ hơn |

### 8.10 CS5C — Ngõ Sau Cơn Mưa

| Mục | Chi tiết |
|---|---|
| **ID** | CS5C |
| **Chapter** | Ch5 |
| **Trigger** | Player quay lại khu dân cư sau gather |
| **Camera** | Dialogue focus nhẹ xuống ngõ thấp/mương nhỏ, không cần cinematic dài |
| **Player control lock** | Có thể có dialogue lock ngắn |
| **NPC positioning** | Cô Hạnh ở đầu ngõ, Em Phúc gần đường đi học |
| **Dialogue** | `CoHanh_EmPhuc_Q5_ResidentialAfterRain` |
| **Objective after** | "Quan sát ngõ sau mưa" |
| **V6 implementation** | `Đã build cinematic pass` — dùng runtime anchor `ResidentialLane`, placeholder interactable cho Cô Hạnh/Em Phúc nếu chưa có model thật |
| **Polish** | Một vài cụm rác thấp/đọng nước giúp người chơi thấy hậu quả quay lại đời sống |

### 8.11 CS5D — Chỗ Sống Nhỏ
...
| **Camera** | Focus gốc cây/bụi cỏ nhỏ |
| **Player control lock** | Dialogue lock ngắn |
| **NPC positioning** | Em Phúc, Bé Na và/hoặc Bác Xanh gần góc cây |
| **Dialogue** | `SmallLives_Q5_Care`, `NeighborhoodPlanting_Q5` |
| **Objective after** | "Dọn góc cây, đặt bát nước và trồng cây non" |
| **V6 implementation** | `Đã build cinematic pass` — reuse placement handler: dọn rác nhỏ, đặt bát nước, đặt biển giữ góc cây sạch |
| **Polish** | Sau khi complete, góc cây thoáng hơn và có một dấu hiệu sự sống quay lại |

### 8.12 CS5E — Lời Hứa Nhà Văn Hóa (Ending)
...
| **Player control lock** | Có |
| **NPC positioning** | Bác Xanh trung tâm. Chị Lan, Cô Tư, Anh Tùng, Ông Sáu, Bé Na, Cô Hạnh, Em Phúc xung quanh. NPC phụ đã gặp đứng phía sau (nếu có) |
...
| **V6 implementation** | `Đã build cinematic pass` — ending route mới qua `Q5_08_ReturnToCommunityHouse`, `EndingSequence`, `PostEnding_FreeRoam`; đã thêm Cô Hạnh/Em Phúc lines và CS5E multi-shot fade |
...
**Nguồn chính:** `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`, `GDD_GreenAgain_v6_IMPLEMENTATION_PLAN.md`
...
- Per-player story state lưu:
  ...
  - `communityProgress` — số NPC đã gather trong `Q5_04_GatherCommunity`
  - `careProgress` hoặc reuse `placementCounts` — số hành động chăm góc sống nhỏ trong `Q5_06_CareSmallLives`
...
- Server tạo runtime placement markers cho prevention/placement actions (Q3_06, Q5_06, Q5_07).
...
- Q5_04: Player quay lại gặp NPC chính (Cô Tư, Anh Tùng, Ông Sáu), sau đó Ch5 tiếp tục qua khu dân cư với Cô Hạnh/Em Phúc.
...
- Ending fires once khi `Q5_08_ReturnToCommunityHouse` complete.
...
**Nguồn chính:** `GDD_GreenAgain_v6_IMPLEMENTATION_PLAN.md`, `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`, `README.md`
...
3. Khi cần chi tiết cụ thể, tham khảo file nguồn:
   - Quest IDs/triggers → `GDD_GreenAgain_v6_QUEST_FLOW.md`
   - Dialogue lines → `GDD_GreenAgain_NPC_DIALOGUE_v6.md`
   - Cutscene staging → `GDD_GreenAgain_v6_CUTSCENE_SCRIPT.md`
   - Story/character → `GDD_GreenAgain_v6_STORY_BIBLE.md`
   - Map → `GDD_GreenAgain_v6_CURRENT_MAP_DRAFT.md`
   - Trash asset library → `GREEN_AGAIN_V6_TRASH_ASSET_LIBRARY.md`

### 11.2 Cách kiểm tra Roblox Studio workspace

1. Mở Roblox Studio, load project Green Again.
2. Trong Explorer, tìm `Workspace["=== GREEN AGAIN V5 ==="]` cho pass hiện tại.
...
1. Trong `StoryRuntimeMVP`, khi quest cleanup bắt đầu:
   ...
   - Set attributes: `GreenAgainV6 = true`, `GreenAgainKind = "Trash"`, `Interactable = true`.
...
- Xác nhận không còn `GreenAgainHUD_V6`, `GreenAgainObjectiveClient`, `MapWiringBootstrap`.
...
2. Sửa `GDD_GreenAgain_v6_CURRENT_MAP_DRAFT.md` trước, rồi sửa code.
...
**Nguồn chính:** `GDD_GreenAgain_v6_IMPLEMENTATION_PLAN.md`, `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md`
...
| `GreenAgainV6` | Map objects | Boolean | Đánh dấu thuộc V6 system |
...
**Nguồn chính:** `GREEN_AGAIN_V6_MVP_IMPLEMENTATION_SUMMARY.md` (Section Test Results), `GDD_GreenAgain_v6_IMPLEMENTATION_PLAN.md` (Section QA)
...
| # | Test Case | Expected Result | Trạng thái |
|---|---|---|---|
| 16 | Old HUD không còn | `GreenAgainHUD_V6` không spawn | ✅ Tested |
...
| # | Test Case | Expected Result | Trạng thái |
|---|---|---|---|
| 23 | Không có text hiện "PicnicArea", "Dam", "Scanner" | Tất cả tên hiển thị đúng tiếng Việt V6 | `Đề xuất bổ sung` — Cần verify toàn bộ |
...
| **Q5_04 Gather — NPC vị trí** | Player quay lại gặp NPC ở đâu? | NPC ở vị trí ban đầu; marker lần lượt dẫn về tạp hóa, sân bóng, bờ sông. |
...
1. **ResidentialLane / SmallLivesCorner coordinates:** Đã đo trong Studio ngày 2026-07-05; đang dùng đoạn `duong` ở cụm nhà dân. Chỉ cần hỏi lại nếu map khu dân cư được remodel.
...
5. **NPC movement:** NPC có cần walk animation, hay chỉ đứng idle?
6. **Visual change sau cleanup:** Map có cần đổi visual (sáng/sạch hơn) sau mỗi chapter?
7. **Post-ending content:** Post-ending có thêm mini-quest giữ Ven Sông xanh không, hay chỉ free roam?

---

> **Document này được tạo bằng cách tổng hợp từ 8 file nguồn V6 + README trong `docs/`. Mọi phần đánh dấu `Đề xuất bổ sung` hoặc `Chưa rõ / cần xác nhận` là do thông tin chưa có trong docs gốc.**
